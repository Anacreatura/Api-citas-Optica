...
export async function createCitas(req, res) {
  try {
...
export async function getOne(req, res) {
  try {
    ...
    if (id.length === 24) { // Longitud válida para un ObjectId
...
export async function addCitas(req, res) {
  try {
    const cita = req.body;
    ...
    if (!buscarCita) {
      ...
      res.status(409).json({ error: "Ya existe una cita creada con esa identificación" });
    }
  } catch (error) {
...
// Eliminar una cita
export async function deleteCitas(req, res) {
  try {
    const id = req.params.id;
...
// Modificar una cita
export async function modificarCitas(req, res) {
  try {
    const id = req.params.id;
    const modificaciones = req.body;

    if (id.length === 24) { // Verificar longitud del ID
      const cita = await Citas.findById(id);
